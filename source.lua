...
function Hide()
	Debounce = true
	RayfieldLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping " .. inputKey01 .."!", Duration = 7})
		...
		function Tab:CreateKeybind(KeybindSettings)
			...
			UserInputService.InputBegan:Connect(function(input, processed)
				...
				if CheckingForKey then
					if input.KeyCode ~= Enum.KeyCode.Unknown and input.KeyCode ~= Enum.KeyCode[inputKey01] then
...
UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode.I and not processed) then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
